...
let fullName = [];
fullName = runners.forEach((list) => {
    console.log(list.first_name, list.last_name);
});
...
let largeShirts = [];
largeShirts = runners.filter((ppl) => {
    return ppl.shirt_size === 'L';
}) 
...
ticketPriceTotal = runners.reduce((total, list) => {
    return total += list.donation;
}, 0)
